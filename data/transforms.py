...
from utils.utils import orig_kspace_to_images
...
class AdvDataTransform:
    """
    Data Transformer for training adversarial attack algorithms. Note that most augmentation will be done after
    the dataloader, so here we just need the k-space actually.
    """
    ...
    def __call__(
        ...
        # i think we have to calculate the target tensor from the kspace, because we need to see more from the target,
        # not only the final center cropped 320x320 image
        kspace_torch = torch.tensor(kspace)
        seed = None if not self.use_seed else tuple(map(ord, fname))
        acq_start = attrs["padding_left"]
        acq_end = attrs["padding_right"]

        acq = [acq_start, acq_end]

        crop_size = (attrs["recon_size"][0], attrs["recon_size"][1])

        # We can create the fully-sized GT from fully-sampled k-space
        target = orig_kspace_to_images(kspace_torch)  #fastmri.ifft2c(kspace_torch)
        # absolute value
        target = torch.abs(target) #fastmri.complex_abs(target)
        ...
        if self.mask_func is not None:
            kspace_torch_for_mask = torch.view_as_real(kspace_torch)
            masked_kspace, mask_torch, num_low_frequencies = apply_mask(
                kspace_torch_for_mask, self.mask_func, seed=seed, padding=(acq_start, acq_end)
            )

            sample = VarNetSample(
                masked_kspace=kspace_torch, # Element is not masked, but VarNetSample can stay constant this way
                mask=mask_torch.to(torch.bool),
                num_low_frequencies=num_low_frequencies,
                target=target, # Target is a fully sized tensor
                fname=fname,
                slice_num=slice_num,
                max_value=max_value,
                crop_size=crop_size,
